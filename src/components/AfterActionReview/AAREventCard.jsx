...
export default function AAREventCard({ chain }) {
  ...
  const { category, delivered_at, prevented_at, trigger_time, title, description, recommendations, skipper_mitigation } = chain;

  const time = formatMs(
    category === 'injected'
      ? delivered_at ?? trigger_time
      : category === 'prevented'
      ? prevented_at ?? trigger_time
      : trigger_time,
  );
  ...
  if (category === 'prevented') {
    ...
    headerLabel = skipper_mitigation?.description
      ? `EVENT PREVENTED thanks to... ${skipper_mitigation.description.toUpperCase()}`
      : 'EVENT PREVENTED';
      ...
      <div className="aar-card__body">
        ...
        {bullets.length > 0 && (
          <div className="aar-card__takeaways">
            <p className="aar-card__takeaways-heading">Key Takeaways:</p>
